...
import { IExecConsumer, ProcessProtectedDataParams } from '@iexec/dataprotector';
import { WorkflowError } from './error';
import { fetchOrdersUnderMaxPrice } from './fetchOrdersUnderMaxPrice';
import { pushRequesterSecret } from './pushRequesterSecret';
...
export const processProtectedData = async ({
  ...
  try {
    ...
    const vApp = addressOrEnsOrAnySchema().required().label('authorizedApp').validateSync(app);
    ...
    let vSecrets;
    if (secrets) {
      vSecrets = secretsSchema().label('secrets').validateSync(secrets);
    }
    const isIpfsStorageInitialized = await iexec.storage.checkStorageTokenExists(requester);
    if (!isIpfsStorageInitialized) {
      const token = await iexec.storage.defaultStorageLogin();
      await iexec.storage.pushStorageToken(token);
    }

    const datasetOrderbook = await iexec.orderbook.fetchDatasetOrderbook(vProtectedData, {
      app: vApp,
      requester,
    });
