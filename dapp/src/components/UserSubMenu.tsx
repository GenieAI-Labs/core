    ...
    <div role='menu' className=''>
      <div className='p-6 text-center' role='none'>
        ...
        <div className='mt-3' role='none'>
          ...
          <p className='text-black mb-4 font-sans text-xs' role='none'>
            {account?.address && truncateAddress(account.address)}
          </p>
          {user && (
            <Link
              href='/dapp/profile/edit'
              className='mt-2 border border-redpraha rounded-xl hover:bg-endnight text-black bg-midnight px-5 py-2 w-full'
              role='none'>
              Manage Account
            </Link>
          )}
